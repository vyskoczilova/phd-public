...
class Helpers:
	...
	@staticmethod
	def get_list_of_annotated_files(parent_directory):
		exclude_directories = ['old', 'test', "_data"]
		...
		for file in directories:
			...
			for f in listdir(source_directory):
				if f in ["N-V_anotovano.csv", "V-N_anotovano.csv"]:
...
class Annotate:
	def __init__(self):
		self.files = Helpers.get_list_of_annotated_files('./chovat_chovávat/')
		self.lemma = []
		self.pos = []
		self.pos_detailed = []
