...
antiexamples_lemma = {
	...
	"z": ["(chyba) Myslí si za pořadatele akce Zuzana Šlúmyslí ", bold, "chová z neziskové organizace Naděje",
		  " Zlín. (V-R-A-N-N)"],
	"k/ke": " ",
}
...
second_position = {
	...
	"P5": ["zájmeno ", italic, "on, oni", " ve tvarech po předložce (tj. ", italic, "n-", ": ", italic, "něj, něho, nich", " …)"],
	"P6": ["reflexívní zájmeno ", italic, "se", " v dlouhých tvarech (", italic, "sebe, sobě, sebou", ")"],
	"P7": ["reflexívní zájmeno ", italic, "se, si", " pouze v těchto tvarech, a dále ", italic, "ses, sis"],
...
# Only POS
only_pos_rows = []
only_pos_columns = ["POS", "příklad", "VNP (%)", "vztažné", "ostatní", "sort_by"]
...
df_only_pos_exclude_vztazne.insert(loc=3, column='kumulativní %',
...
pos_detailed_rows = []
pos_detailed_columns = ["POS", "POS + detail", "VNP", "vztažné", "ostatní", "délka"]
...
df_pos_detailed = df_pos_detailed.sort_values("VNP", ascending=False)

# POS & POS with second position - interested only in the middle
pos_detailed_simplified_rows = []
pos_detailed_simplified_columns = ["POS", "POS + detail", "VNP", "vztažné", "ostatní", "délka"]
...
df_pos_detailed_simplified = df_pos_detailed_simplified.sort_values("VNP", ascending=False)
	...
	if row["POS"] in at_analyzed:
		...
		for p_d in pos_detailed:
			if p_d not in at_pos_d:
				at_pos_d[p_d] = 0
				at_pos_d_counts[p_d] = {}
			at_pos_d[p_d] += row["VNP"]
			at_pos_d_counts[p_d][row["POS + detail"]] = row["VNP"]
...
pron_columns = ["detailní určení", "příklad", "VNP", "%", "sort_by"]
pron_rows = []
...
prep_columns = ["předl.", "příklad", "VNP (%)", "protipříklad", "ostatní", "sort_by"]
prep_rows = []
...
for lemma, count in prep_data_is_vnp.items():
	...
	prep_rows.append(
		[lemma, example, str(count) + prcnt_wrap(count, prep_total_is_vnp), antiexample, count_non_vnp, count])
